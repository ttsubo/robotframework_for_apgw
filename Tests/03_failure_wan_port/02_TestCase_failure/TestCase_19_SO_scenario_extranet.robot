...
*** TestCases ***
(19-1) Create Vrf(V06010001) in GateSW#2
...
(19-2) Check Status of Create_Vrf
    Wait Until Keyword Succeeds  30s  10s
    ...  Check Status Create_Vrf
    ...  ${VRF_UUIDS['${nw_user_id}']}
    ...  ${expected_status}

(19-3) Create Interface(30.1.0.1/24) in VLAN(2001) in Vrf(V06010001)
...
(19-4) Check Status of Create_Interface
    Wait Until Keyword Succeeds  30s  10s
...
(19-5) Check Connectivity From GateSW to vFirewall(Primary)
...
(19-6) Check Connectivity From GateSW to vFirewall(Secondary)
    Wait Until Keyword Succeeds  60s  10s
    ...  Check Connectivity
    ...  ${VRF_UUIDS['${test_num_previous}']}
    ...  ${ipv4_vfw2}
...
(19-7) Check Connectivity From GateSW to remotePrefix(192.168.200.1)
    Wait Until Keyword Succeeds  60s  10s
    ...  Check Connectivity
    ...  ${VRF_UUIDS['${test_num_previous}']}
    ...  ${remotePrefix}
...
(19-8) Create Route(40.1.1.0/24) in Vrf(V06010001)
...
(19-9) Check Status of Create_Route
    Wait Until Keyword Succeeds  30s  10s
...
(19-10) Check Connectivity From GateSW to Local_vm(40.1.1.1)
    Wait Until Keyword Succeeds  60s  10s
    ...  Check Connectivity
    ...  ${VRF_UUIDS['${test_num_previous}']}
    ...  ${local_vm}
...
(19-11) Update Vrf(V06010001) Extranets
    ${extranets}=  Create List  ${extranet_user_id}
    ${vrf}=  Create Dictionary  extranets=${extranets}
    Update Vrf Extranets  ${VRF_UUIDS['${nw_user_id}']}
                                  ...  ${vrf}

(19-12) Check Status of Update_Vrf for extranet
    Wait Until Keyword Succeeds  30s  10s
    ...  Check Status Update_Vrf
    ...  ${VRF_UUIDS['${nw_user_id}']}
    ...  ${expected_status}

(19-13) Check Connectivity From GateSW to vFirewall(Primary) for extranet
    Wait Until Keyword Succeeds  60s  10s
    ...  Check Connectivity
    ...  ${VRF_UUIDS['${nw_user_id}']}
    ...  ${ipv4_vfw1_extra}
...
(19-14) Check Connectivity From GateSW to vFirewall(Secondary) for extranet
    Wait Until Keyword Succeeds  60s  10s
    ...  Check Connectivity
    ...  ${VRF_UUIDS['${nw_user_id}']}
    ...  ${ipv4_vfw2_extra}
